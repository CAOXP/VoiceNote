...
void loop() {
  if (STTclient.available()) {       //监听讯飞语音识别服务器
    STTclient.poll();
  }
  if (TTSclient.available()) {        //监听讯飞语音合成服务器
    TTSclient.poll();
  }    
  if(downbutton.onPressed()){     //如果向下拨键
    position++;
    if(position>4){
      position = 1;
    }
  }
  if(upbutton.onPressed()){      //如果向上拨键
    position--;
    if(position<1){
      position = 4;
    }
  ...
  if(enterbutton.onPressed()){
    if(menu_level != 0 && position == 4){    //如果按下拨键
      menu_level--;
      position = 1;
    }else{
      menu_level++;
  ...
  if(menu_level==1 && L0_position==1){
    ...
    if(STT_state==1 && GPT_state==1){         //进入“开始提问”
      Serial.println("开始提问"); 
      while(1){
        question_page();
        ...
        if (enterbutton.onPressed()) {
          ...
          buttonsignal=1;
          speech_page();
          if(textlength<150){         //对大模型回答文本进行处理
            TTSsignal=1;
            postTTS(answer);              
            while(TTSsignal==1){
  ...
  if(menu_level==1 && L0_position==4){     //进入使用教程页面
    videoQRcode(); 
    while(1){
      delay(10);
      if(enterbutton.onPressed()){
  ...
  if(menu_level==2 && L1_position==1){       //进入信息输入页面
    u8g2.clearBuffer(); 
    u8g2.setDrawColor(1); 
  ...
  if(menu_level==2 && L1_position==2){     //进入音量设置页面
    int vol_val;
    byte read_val =EEPROM.read(111);
    if(read_val == EMPTY_MARKER){
      vol_val=5;
    }else{vol_val = EEPROM.read(111);}
  ...
  if(menu_level==2 && L1_position==3){        //进入语速设置页面
    int spd_val;
    byte read_val =EEPROM.read(112);
    if(read_val == EMPTY_MARKER){
      spd_val=5;
    }else{spd_val = EEPROM.read(112);}
  ...
  if(menu_level==2 && L1_position==4){       //进入当前使用大模型页面
  ...
  if(menu_level==2 && L1_position==6){            //进入发送邮件页面
  ...
  if(menu_level==3 && L2_position==1){    //设置使用的大模型
    EEPROM.write(110,1);
    EEPROM.commit();
    u8g2.clearBuffer(); 
    u8g2.setDrawColor(1); 
  ...
  if(menu_level==3 && L2_position==2){      //设置使用的大模型
    EEPROM.write(110,2);
    EEPROM.commit();
    u8g2.clearBuffer(); 
    u8g2.setDrawColor(1); 
  ...
  if(menu_level==3 && L2_position==3){     //设置使用的大模型
    EEPROM.write(110,3);
    EEPROM.commit();
    u8g2.clearBuffer(); 
    u8g2.setDrawColor(1); 
...
void handleConnect() {                //手机上输入信息并提交后，语音助手接收信息并保存
...
String getAliAnswer(String inputText){      //调用阿里问答大模型
...
String getmaxAnswer(String inputText){          //调用miniMax问答大模型
...
String getSparkAnswer(String inputText){          //监听讯飞星火大模型
  Sparksignal=1;
  backtext="";
  postSpark(inputText);
  while(Sparksignal==1){
...
void emailSendHtml(String textcontent){          //发送邮件 
  /* 定义smtp message消息类 */
...
void EEPROMwrite(String EEname,int EEpoistion){          //写入存储数据
...
String EEPROMread(int EEposition){               //读取存储数据
  String EEname = "";
...
void showTime(){           //将时间数据设置为讯飞服务器要求的格式
  struct tm timeinfo;
...
String xunufeiUrl(String hostPart, String requestLine, String url){               //讯飞web服务器握手程序
  showTime();
...
void postSpark(String textspark){                                   //调用讯飞星火大模型
...
void postTTS(String texttts) {                         //调用讯飞语音合成
  ...
  if(read_val == EMPTY_MARKER){
    TTSvol=50;
  }else{TTSvol = EEPROM.read(111)*10;}
  read_val =EEPROM.read(112);
  if(read_val == EMPTY_MARKER){
    TTSspd=50;
  }else{TTSspd = EEPROM.read(112)*10;}
